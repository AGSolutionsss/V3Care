...
export default class NewListArea extends React.Component {
  state = {
    loader: true,
    users: [],
    areaData: [],
  ...
  getData = () => {
    let result = [];
    axios({
      url: baseURL+"/panel-fetch-area-list",
      method: "GET",
      headers: {
      ...
      .then((res) => {
        ...
        for (let i = 0; i < response.length; i++) {
            ...
            tempRows.push([
              ...
              response[i]["area"],
              response[i]["area_status"],
              response[i]["id"],
            ]);
          
        }
        this.setState({ areaData: tempRows, loader: false });
      })
      .catch((res) => {
