...
export default class NewListConfirmedBooking extends React.Component {
  state = {
    loader: true,
    showmodal: false,
    bookingid: '',
  ...
  getData = () => {
    let result = [];
    axios({
      url: baseURL+"/panel-fetch-booking-confirmed-list",
      method: "GET",
      headers: {
      ...
      .then((res) => {
        ...
        for (let i = 0; i < response.length; i++) {
            ...
            tempRows.push([
              <span style={{padding:'10px',borderRadius:'50%',color:'#fff',fontWeight:'600',backgroundColor:(response[i]["order_status"] == 'Confirmed' ? 'red': response[i]["order_status"] == 'On the way' ? 'violet' : 'green')}}>{response[i]["order_status"].trim().charAt(0)}</span>,
