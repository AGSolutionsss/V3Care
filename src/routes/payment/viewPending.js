...
const ViewPending = (props) => {
    ...
    const updateData = (e) => {
        e.preventDefault();
        let data = {
          order_check_payment_type : payment.order_check_payment_type,
          order_check_payment_details: payment.order_check_payment_details,
        }
        var url = new URL(window.location.href);
    var id = url.searchParams.get("id");
        axios({
          url: baseURL+"/panel-update-payment-status/"+id,
          method: "PUT",
          data,
          headers: {
            Authorization: `Bearer ${localStorage.getItem("login")}`,
  ...
  return (
    <div className="textfields-wrapper">
      <PageTitleBar title="View Pending Payment" match={props.match} />
